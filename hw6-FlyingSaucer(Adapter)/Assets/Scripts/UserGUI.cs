	...
	{	
		if(action.GetModeSetting()){
			...
			if(action.GetRound() == 1){
				GUI.Label(new Rect(150, 10, 200, 50), "ROUND");
				GUI.Label(new Rect(200, 10, 200, 50), action.GetRound().ToString());
				GUI.Label(new Rect(150, 30, 300, 50), "分数到达5分进入下一关，打中一个飞碟得1分，错过一个飞碟生命值减1，生命值归零时游戏失败。");
			}
			...
			if(GUI.Button(new Rect(500, 10, 80, 40), "Pause")){
				go = false;
			}
			if(go == false){
			...
			if (GUI.Button(new Rect(600, 10, 80, 40), "Restart")){
				action.Restart();
				return;
			}
